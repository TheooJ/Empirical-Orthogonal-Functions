...
def probe_intensity(probe, actuators=None):
    ...
    # dtype = 'complex'
    E0 = hcipy.Field(np.ones(aperture_1.size), aperture_1) * 1 * np.ones(
        aperture_1.size)  # * np.random.normal(size=aperture.size)
...
plt.figure()
hcipy.imshow_field(reshape_dark_hole_to_aperture_intensity((np.abs(E_actual - E_estimated) ** 2)),
